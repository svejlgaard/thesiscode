...
elements = ['SII','SiII', 'AlII', 'FeII']
plotelements = ['SII','SiII','MgII','FeII', 'OI','AlII', 'CII']
alphaelements = ['SII', 'SiII', 'MgII', 'AlII']
...
def get_xy(elem):
    ...
    a = a.to_numpy()[:,0]
    alphaenhance = np.isin(elem, alphaelements, invert=False).astype(int)
    yvals = logN - solar + 12 - 0.3*alphaenhance - a - obs_logNH
...
plt.xlabel('x')
plt.ylabel('y')
#plt.savefig(f'figures/DynestyFittingResults_alpha.pdf')
...
plt.tight_layout()
plt.savefig('../../Text/figures/mn_corner_alpha.pdf')
...
for i, txt in enumerate(plotelements):
    if txt == 'SiII':
        ax.annotate(txt.replace('I',''), (xplot[i]-0.15, yplot[i]-0.06))
    elif txt == 'MgII':
        ax.annotate(txt.replace('I',''), (xplot[i]-0.21, yplot[i]+0.02))
    elif txt == 'SII':
        ax.annotate(txt.replace('I',''), (xplot[i]-0.09, yplot[i]+0.02))
    elif txt == 'FeII':
        ax.annotate(txt.replace('I',''), (xplot[i]+0.02, yplot[i]+0.02))
    elif txt == 'AlII':
        ax.annotate(txt.replace('I',''), (xplot[i]+0.02, yplot[i]-0.06))
    else:
        ax.annotate(txt.replace('I',''), (xplot[i]+0.03, yplot[i]+0.02))
ax.set(xlim=(-1.75, 0.1), ylim=(-2.4,-1.5), xlabel='x', ylabel='y')
plt.tight_layout()
plt.savefig(f'../../Text/figures/DepletionFittingResults_alpha.pdf')
