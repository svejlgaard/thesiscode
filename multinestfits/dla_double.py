...
def power(theta):
    f0, L, n1, n2, av = theta
    return fit_wave[incl]**(L-3.5) * f0 * 1e-10 * addAbs(fit_wave[incl],10**(n1),zabs) * addAbs(fit_wave[incl],10**(n2),zabs2) * 10**(-0.4*aLambda(fit_wave[incl],av))

def power_init(x, f0, L, n1, n2, av):
    return x**(L-3.5) * f0 * 1e-10 * addAbs(x,10**(n1),zabs) * addAbs(x,10**(n2),zabs2) * 10**(-0.4*aLambda(x,av))


# Defining the prior
def prior(utheta):
    uf0, uL, un1, un2, uav = utheta
    f0 = uf0
    L = L_sigma*uL + L_init
    n1 = 3*un1 + 19
    n2 = 3*un2 + 19
    av = uav
    return f0, L, n1, n2, av
...
parameters = [r'$F_0$',r'$\Gamma$',r'log(N$_{DLA,1}$/cm$^{-2})$', r'log(N$_{DLA,2}$/cm$^{-2})$', r' a$_V$/mag']
...
#plt.savefig('../figures/dla_double_zoom.pdf')

# PLOTTING THE QUANTILES IN CORNER PLOT
plt.rcParams['axes.labelsize'] = 12
plt.rcParams['xtick.labelsize'] = 12
plt.rcParams['ytick.labelsize'] = 12
plotsamples = samples[2:5]
df = pd.DataFrame(data=plotsamples.T, columns=[r'log(N$_{DLA,1}$/cm$^{-2})$', r'log(N$_{DLA,2}$/cm$^{-2})$', r' a$_V$/mag'])
...
grid.figure.axes[8].axvline(x=res_array[4,0]+res_array[4,1], color='k', linestyle='dashed')
grid.figure.axes[0].set_title(parameters[2]+' = '+f'{res_array[2,0]:.2f}'+r'$\pm$'+f'{res_array[2,1]:.2f}')
grid.figure.axes[2].set_title(parameters[3]+' = '+f'{res_array[3,0]:.2f}'+r'$\pm$'+f'{res_array[3,1]:.2f}')
grid.figure.axes[8].set_title(parameters[4]+' = '+f'({res_array[4,0]*1e3:.2f}'+r'$\pm$'+f'{res_array[4,1]*1e3:.2f})'+r'$\times$10$^{-3}$')
plt.tight_layout()
#plt.savefig('../figures/dla_double_corner.pdf')